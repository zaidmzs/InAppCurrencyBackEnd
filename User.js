...
exports.handler = async function(event) {
  ...
  switch(true) {
    ...
    case event.httpMethod === 'GET' && event.path === userPath:
      response = await getUser(event.queryStringParameters.MobileNumber);
      break;
    ...
    case event.httpMethod === 'PATCH' && event.path === userPath:
      ...
      response = await modifyUser(requestBody.MobileNumber, requestBody.updateKey, requestBody.updateValue);
      break;
    case event.httpMethod === 'DELETE' && event.path === userPath:
      response = await deleteUser(JSON.parse(event.body).MobileNumber);
      break;
    default:
...
async function modifyUser(MobileNumber, updateKey, updateValue) {
  const params = {
    TableName: dynamodbTableName,
    Key: {
      'MobileNumber': MobileNumber
